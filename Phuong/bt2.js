...
const daysUntil2023 = (dateString) => {
  ...
  return `${Math.abs((firstDate2023.getTime() - inputDate.getTime()) / 86400000)} days`
}
...
const weeklySalary = (array) => {
  ...
  array.forEach((hours, index) => {
    ...
    if (index === array.length - 1 || index === array.length - 2) daySalary *= 2
...
const mapLetters = (string) => {
  ...
  instincArr.forEach(item => {
    const indices = array.map((char, index) => char === item ? index : '').filter(String)
    result[item] = indices
  })
  return result
}
